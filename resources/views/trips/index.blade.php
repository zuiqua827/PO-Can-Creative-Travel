...
<div class="bg-slate-900 py-10 border-b border-slate-800 text-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h1 class="text-3xl font-black tracking-tight mb-4">Cari & Pesan Jadwal Bus</h1>
        ...
        <form action="{{ route('trips.index') }}" method="GET" class="bg-white/10 backdrop-blur-md p-4 sm:p-5 rounded-2xl border border-white/15 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
            <div>
                <label class="block text-[11px] font-bold uppercase text-slate-300 mb-1">Dari</label>
                <select name="origin" class="w-full py-2.5 px-3 rounded-xl bg-slate-800 border border-slate-700 text-white text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-brand-500">
            ...
            <div>
                <label class="block text-[11px] font-bold uppercase text-slate-300 mb-1">Tujuan</label>
                <select name="destination" class="w-full py-2.5 px-3 rounded-xl bg-slate-800 border border-slate-700 text-white text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-brand-500">
            ...
            <div>
                <label class="block text-[11px] font-bold uppercase text-slate-300 mb-1">Tanggal</label>
            ...
            <div>
                <label class="block text-[11px] font-bold uppercase text-slate-300 mb-1">Kelas Bus</label>
                <select name="bus_type" class="w-full py-2.5 px-3 rounded-xl bg-slate-800 border border-slate-700 text-white text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-brand-500">
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-6 border-b border-slate-200 gap-4 mb-6">
        <div class="text-sm text-slate-600 font-medium">
            ...
            @if($origin) untuk <span class="text-brand-600 font-bold">{{ $origin }}</span> @endif
            @if($destination) ke <span class="text-brand-600 font-bold">{{ $destination }}</span> @endif
        </div>

        <!-- Sort Select -->
        <div class="flex items-center space-x-2 text-xs">
            <span class="text-slate-500 font-bold uppercase">Urutkan:</span>
            <form action="{{ route('trips.index') }}" method="GET" id="sortForm">
                ...
                <select name="sort" onchange="document.getElementById('sortForm').submit()" 
                    class="py-1.5 px-3 rounded-xl border border-slate-200 bg-white text-slate-800 font-semibold focus:outline-none focus:ring-2 focus:ring-brand-500">
    ...
    <div class="space-y-5">
        @forelse($trips as $trip)
            <div class="bg-white rounded-3xl p-6 sm:p-7 border border-slate-200/80 shadow-sm hover:shadow-xl transition-all duration-300">
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
                    ...
                    <div class="lg:col-span-4 space-y-2">
                        ...
                        <h3 class="text-lg font-black text-slate-900">{{ $trip->bus->name }}</h3>
                    ...
                    <!-- Middle: Timeline Origin & Destination -->
                    <div class="lg:col-span-5 border-y lg:border-y-0 lg:border-x border-slate-100 py-4 lg:py-0 lg:px-6">
                        <div class="flex items-center justify-between">
                            <div>
                                <span class="text-[10px] font-bold text-slate-400 uppercase">Berangkat</span>
                                <div class="text-2xl font-black text-slate-900">{{ $trip->departure_at->format('H:i') }} <span class="text-xs font-normal text-slate-400">WIB</span></div>
                            ...
                            <div class="flex flex-col items-center px-2">
                                <span class="text-[10px] font-bold text-brand-600 bg-brand-50 px-2 py-0.5 rounded-full mb-1">
                            ...
                            <div class="text-right">
                                <span class="text-[10px] font-bold text-slate-400 uppercase">Tiba</span>
                                <div class="text-2xl font-black text-slate-900">{{ $trip->arrival_at->format('H:i') }} <span class="text-xs font-normal text-slate-400">WIB</span></div>
                        ...
                        <div class="mt-3 text-[11px] text-slate-500 flex justify-between">
                            <span class="truncate">📍 Titik Naik: <strong>{{ $trip->boarding_point ?: $trip->route->origin }}</strong></span>
                        </div>
                    </div>
                    ...
                    <div class="lg:col-span-3 text-right flex flex-col justify-between items-end space-y-4">
                        <div>
                            <span class="text-xs text-slate-400 block">Harga per Penumpang</span>
                        ...
                        <div class="w-full">
                            @if($trip->available_seats_count > 0)
                                <a href="{{ route('trips.show', $trip) }}" class="w-full flex items-center justify-center py-3 px-5 rounded-2xl bg-gradient-to-r from-brand-600 to-brand-700 hover:from-brand-500 hover:to-brand-600 text-white font-bold text-sm shadow-md shadow-brand-600/25 transition">
        ...
        @empty
            <div class="text-center py-16 bg-white rounded-3xl border border-slate-200 p-8">
                ...
                <h3 class="text-lg font-bold text-slate-800">Tidak ada jadwal perjalanan yang cocok</h3>
                <p class="text-sm text-slate-500 max-w-md mx-auto mt-1 mb-6">
                    Silakan ubah tanggal atau pilih rute kota asal dan tujuan lainnya untuk melihat jadwal bus PO CAN Travel.
                </p>
                <a href="{{ route('trips.index') }}" class="inline-flex items-center px-4 py-2.5 rounded-xl bg-slate-900 text-white text-xs font-bold hover:bg-slate-800 transition">
                    Reset Semua Filter
                </a>
            </div>
        @endforelse
